...
public class AddReceiver {
	...
	public static ObservableList<Company> receivers = FXCollections.observableArrayList();
	...
	public VBox getPage() {
		...
		Label label = new Label("Add New Receiver:");
		...
		add.setOnAction(e -> {
			...
			// If all fields are filled in
				...
				alert1.setContentText("Receiver Added.");
				alert1.show();
			}
		});
		...
		Label deleteLabel = new Label("Select Receiver:");
		...
		receivers.addListener((ListChangeListener.Change<? extends Company> e) -> {
			if (adding) {
				list.getItems().add(receivers.get(receivers.size() - 1).getName());
			}
		});
		Button deleteButton = new Button("Delete Receiver");
			...
			try {
				...
				if (delItem.length() > 0) {
					...
					if (alert.getResult() == ButtonType.OK) {
						...
						CreateInvoice.receiverList.getItems().remove(list.getSelectionModel().getSelectedIndex());
						...
						for (Company comp : receivers) {
							if (comp.getName().equals(delItem)) {
								...
								manage.deleteReceiver(comp);
								receivers.remove(comp);
								break;
							}
						}
						// Reset the labels
