...
public class AddContact {
	...
	public static ObservableList<Company> contacts = FXCollections.observableArrayList();
	...
	public VBox getPage() {
		...
		Label label = new Label("Add New Broker:");
		...
		Button add = new Button("Add Broker");
		...
		add.setOnAction(e -> {
			...
			// If all fields are filled in
				...
				alert1.setContentText("Contact Added.");
				alert1.show();
			}
		});
		...
		Label deleteLabel = new Label("Select Broker:");
		...
		contacts.addListener((ListChangeListener.Change<? extends Company> e) -> {
			if (adding) {
				list.getItems().add(contacts.get(contacts.size() - 1).getName());
			}
		});
		Button deleteButton = new Button("Delete Broker");
			...
			try {
				...
				if (delItem.length() > 0) {
					...
					if (alert.getResult() == ButtonType.OK) {
						// Remove the item from the ChoiceBox in Create Invoice page
						CreateInvoice.brokerList.getItems().remove(list.getSelectionModel().getSelectedIndex());

						// Remove item from ChoiceBox in Contacts page
						...
						for (Company comp : contacts) {
							if (comp.getName().equals(delItem)) {
								...
								manage.deleteContact(comp);
								contacts.remove(comp);
								break;
							}
						}
						// Reset the labels
